...
const CGFloat kGHRevealSidebarWidth = 110;
...
@synthesize isShowIcon;

-(void) setIsShowIcon:(BOOL)pisShowIcon{
    isShowIcon=pisShowIcon;
    _icon.hidden=!isShowIcon;
}
...
- (void)setContentViewController:(UIViewController *)cvc {
    ...
    if(_icon){
        [_contentView bringSubviewToFront:_icon];
    }
}
...
- (void)viewDidLoad {
    ...
    _icon=[[UIImageView alloc] initWithFrame:CGRectMake(-2, (self.view.bounds.size.height-48)/2.0, 20, 48)];
    _icon.image=[UIImage imageNamed:@"menuicon.png"];
...
- (void)dragContentView:(UIPanGestureRecognizer *)panGesture {
	...
	if (panGesture.state == UIGestureRecognizerStateChanged) {
		if (sidebarShowing) {
			if (translation > 0.0f) {
				_contentView.frame = CGRectOffset(_contentView.bounds, screenWidth-kGHRevealSidebarWidth, 0.0f);
				self.sidebarShowing = YES;
			} else if (translation < -(screenWidth-kGHRevealSidebarWidth)) {
				_contentView.frame = _contentView.bounds;
				self.sidebarShowing = NO;
			} else {
				_contentView.frame = CGRectOffset(_contentView.bounds, ((screenWidth-kGHRevealSidebarWidth) + translation), 0.0f);
			}
		} else {
			if (translation < 0.0f) {
				_contentView.frame = _contentView.bounds;
        ...
        if(self.sidebarShowing){
            if(translation<kGHRevealSidebarWidth){
                show = (fabs(velocity) > kGHRevealSidebarFlickVelocity) ? (velocity > 0) : (fabs(translation)+kGHRevealSidebarWidth < ((screenWidth-kGHRevealSidebarWidth) / 2));
            }else{
                show = YES;
            }
        }else{
            show = (fabs(velocity) > kGHRevealSidebarFlickVelocity) ? (velocity > 0) : (translation > ((screenWidth-kGHRevealSidebarWidth) / 2));
        }
...
- (void)toggleSidebar:(BOOL)show duration:(NSTimeInterval)duration completion:(void (^)(BOOL finsihed))completion {
	void (^animations)(void) = ^{
		if (show) {
			_contentView.frame = CGRectOffset(_contentView.bounds, (screenWidth-kGHRevealSidebarWidth), 0.0f);
            [_contentView addSubview:_gestureView];
            [_gestureView addGestureRecognizer:_tapRecog];
//			[_contentView addGestureRecognizer:_tapRecog];
		} else {
			if (self.isSearching) {
				_sidebarView.frame = CGRectMake(0.0f, 0.0f, (screenWidth-kGHRevealSidebarWidth), CGRectGetHeight(self.view.bounds));
			} else {
//				[_contentView removeGestureRecognizer:_tapRecog];
                [_gestureView removeFromSuperview];
                [_gestureView removeGestureRecognizer:_tapRecog];
			}
			_contentView.frame = _contentView.bounds;
		}
